...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 8% 0,#fff7df,transparent 34%),linear-gradient(145deg,var(--bg),#ffffff 58%,#efe7d8);color:var(--ink);font-family:"Bahnschrift","Aptos Display","Trebuchet MS",sans-serif}
    ...
    button{border:0;border-radius:16px;padding:12px 16px;font-weight:800;color:white;background:linear-gradient(135deg,var(--blue),var(--ok));cursor:pointer}button.secondary{background:#fff;color:var(--ink);border:1px solid var(--line)}button.danger{background:linear-gradient(135deg,#ff8a8a,var(--bad))}
...
<body>
  ...
  <script>
    ...
    $('providerForm').onsubmit=e=>{e.preventDefault();const b=formData(e.target);if(!b.api_password)delete b.api_password;api('/api/admin/providers',{method:'POST',body:JSON.stringify(b)}).then(x=>show(x)).catch(e=>show(e.message))}
    $('serverForm').onsubmit=e=>{e.preventDefault();const b=formData(e.target);b.name=b.name||'服务器 #'+b.id;b.enabled=b.enabled==='true';b.daily_reboot_limit=Number(b.daily_reboot_limit||3);b.check_method='api_only';api('/api/admin/servers',{method:'POST',body:JSON.stringify(b)}).then(refresh).catch(e=>show(e.message))}
