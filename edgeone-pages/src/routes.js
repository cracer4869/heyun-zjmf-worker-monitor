...
async function githubUpdateConfig(repo, env) {
  ...
  return {
    ...
    workflow: await repo.getSetting('github_workflow_file', env.GITHUB_WORKFLOW_FILE || 'edgeone-monitor-cron.yml'),
...
function createRunLeaseOwner(now) {
  const randomPart = typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
    ? crypto.randomUUID()
    : Math.random().toString(36).slice(2);
  return `edgeone-run-${now}-${randomPart}`;
}
...
export async function handleRequest(request, env) {
  ...
  const monitorFetcher = env.fetcher || ((input, init) => fetch(input, init));
  ...
  if (url.pathname.startsWith('/api/admin/run/') && request.method === 'POST') {
    const serverId = decodeURIComponent(url.pathname.slice('/api/admin/run/'.length));
    const existing = serverId && typeof repo.getServer === 'function' ? await repo.getServer(serverId) : null;
    if (!existing) return json({ error: 'SERVER_NOT_FOUND' }, 404);
    const now = Math.floor(Date.now() / 1000);
    const result = await runMonitorOnce({
      repo,
      fetcher: monitorFetcher,
      tcpConnector: env.tcpConnector,
      now,
      force: true,
      targetServerIds: [serverId],
      allowActions: false,
    });
    return json({ ...result, server_id: serverId });
  }

  if (url.pathname === '/api/admin/run' && request.method === 'POST') {
    const now = Math.floor(Date.now() / 1000);
    const force = boolValue(url.searchParams.get('force'));
    const leaseOwner = createRunLeaseOwner(now);
    if (typeof repo.acquireRunLease === 'function') {
      const lease = await repo.acquireRunLease(leaseOwner, now, 600);
      if (!lease.acquired) {
        return json({ checked: 0, skipped: true, reason: 'RUN_IN_PROGRESS', owner: lease.owner, expires_at: lease.expires_at });
      }
    }
    try {
      return json(await runMonitorOnce({
        repo,
        fetcher: monitorFetcher,
        tcpConnector: env.tcpConnector,
        now,
      ...
      if (typeof repo.releaseRunLease === 'function') await repo.releaseRunLease(leaseOwner);
    }
  }

  if (url.pathname === '/api/admin/notify/test' && request.method === 'POST') {
    const notifier = new Notifier(await repo.getSettings(), monitorFetcher);
