...
export async function runMonitorOnce({
  ...
  const targetIds = Array.isArray(targetServerIds) && targetServerIds.length
    ? new Set(targetServerIds.map(String))
    : null;
  const servers = (await repo.listEnabledServers()).filter((server) => !targetIds || targetIds.has(String(server.id)));
  let checked = 0;
  ...
  for (const server of servers) {
    ...
    if (allowActions && shouldReboot(nextRuntime, server, settings, now, rebootWindow, recentRebootCount)) {
